...
function receberPorcentagem(numero) {
    ...
    if (numero == 0) {
        gorjeta = document.querySelector("#tip-input").value
        validarDados()
        return
    }
...
function calcularTotais() {
   ...
   const paragrafoGorjeta = document.querySelector("#gorjeta-total")
   paragrafoGorjeta.innerText = `$${gorjetaPorPessoa.toFixed(2)}`

   const totalPorPessoa = (conta / numeroDePessoas) + gorjetaPorPessoa
   const paragrafoTotal = document.querySelector("#conta-total")
